  ...
  const StickyNote = ({ note }) => {
    ...
    return (
      ...
      >
        ...
        <div className="bg-[#d9e87b] p-6 pt-8 rounded-sm shadow-2xl relative overflow-hidden flex flex-col min-h-[300px] border-b-4 border-r-4 border-black/10">
           {/* Line pattern */}
           <div className="absolute inset-0 bg-[linear-gradient(transparent_27px,#94a348_28px)] bg-[length:100%_28px] opacity-20 pointer-events-none" />
           <div className="absolute left-10 top-0 bottom-0 w-px bg-rose-400 opacity-30 pointer-events-none" />

           <div className="relative z-10 flex flex-col h-full">
              <h4 className="text-[10px] font-black text-[#5a6b1d] uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                 Personal Notes
              </h4>
              
              <p className="text-[#3c4a14] font-medium leading-[28px] flex-1 mb-6 text-sm">
                {note.content}
              </p>

              <div className="flex flex-wrap gap-2 mb-6">
                 {note.tags.map(tag => (
                   <span key={tag} className="px-3 py-1 rounded-full bg-[#5a6b1d] text-[#d9e87b] text-[10px] font-black uppercase tracking-wide">
                     # {tag}
                   </span>
                 ))}
              </div>
              ...
              <div className="border-t border-[#5a6b1d]/20 pt-4 flex flex-col gap-3">
                 <button 
                   ...
                   className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${note.convertedToTask ? 'text-[#8a9b4d]' : 'text-[#5a6b1d] hover:translate-x-1 transition-transform'}`}
                 >
                 ...
                 <div className="flex items-center justify-between mt-2">
                    <span className="text-[9px] font-bold text-[#5a6b1d]/60 flex items-center gap-1">
                       <Clock size={10} /> Saved {timeAgo(note.createdAt)}
                    </span>
                    <div className="flex gap-2">
                       <button className="w-8 h-8 rounded-lg bg-[#5a6b1d]/10 flex items-center justify-center text-[#5a6b1d] hover:bg-[#5a6b1d]/20 transition-all">
                          <Sparkles size={14} />
                       </button>
                       <button 
           ...
           {/* Corner fold effect */}
           <div className="absolute bottom-0 right-0 w-12 h-12 bg-[#c4d36a] rounded-tl-sm shadow-[-5px_-5px_15px_rgba(0,0,0,0.05)]" />
        </div>
      </motion.div>
    );
  };
